...
use bevy_render::{
    ...
    renderer::{ViewQuery, RenderContext},
    sync_world::RenderEntity,
    view::{ExtractedView, ViewTarget},
};

/// Exclusive system that processes paint callback updates before the render pass.
pub fn egui_paint_callback_update(world: &mut World) {
...
/// Egui render pass system. Replaces the old `EguiPassNode` render graph node.
pub fn egui_pass(
    world: &World,
    view: ViewQuery<&EguiCameraView>,
    egui_view_query: Query<(&ExtractedView, &EguiViewTarget)>,
    egui_view_target_query: Query<(&ViewTarget, &ExtractedCamera)>,
    egui_pipelines: Res<EguiPipelines>,
    pipeline_cache: Res<PipelineCache>,
    render_data: Res<EguiRenderData>,
    bind_groups: Res<EguiTextureBindGroups>,
    egui_transforms: Res<EguiTransforms>,
    mut ctx: RenderContext,
) {
    let egui_camera_view = view.into_inner();
    let egui_view_entity = egui_camera_view.0;

    let Ok((extracted_view, view_target)) = egui_view_query.get(egui_view_entity) else {
        return;
    };

    let Ok((target, camera)) = egui_view_target_query.get(view_target.0) else {
        return;
    };

    let Some(data) = render_data.0.get(&extracted_view.retained_view_entity.main_entity) else {
            ...
            DrawPrimitive::PaintCallback(command) => {
                ...
                if viewport.width_px > 0 && viewport.height_px > 0 {
                    ...
                    command.callback.cb().render(
                        info,
                        &mut render_pass,
                        RenderEntity::from(egui_view_entity),
                        pipeline_key,
                        world,
                    );
                }
